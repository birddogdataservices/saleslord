# ProspectLord — Claude Code context

## Platform context

ProspectLord is one of two apps in the **SalesLord platform** (the other is CELord).
It lives in the same repo and shares the same Supabase project, auth setup, and Vercel
deployment as CELord during Stage 1. See the root `CLAUDE.md` for shared stack, env vars,
Next.js 16 quirks, and deployment details. See `docs/celord/CLAUDE.md` for CELord context.

**Rename path:** This app is currently named SalesLord in code and user-facing strings.
It will be renamed ProspectLord at Stage 2 when the repo is restructured into a proper
monorepo. Until then, "SalesLord" and "ProspectLord" refer to the same app.

## What this is

A B2B sales prospecting assistant for enterprise AEs. It replaces the gap between "here's
a company profile" and "here's what to actually say to this specific person today." Every
output — briefs, emails, follow-ups, suggested angles — is grounded in the rep's voice,
the prospect's fiscal year and buy cycle, and real relationship history.

Not a sequencing tool. Does not automate outreach. Intelligence and drafting layer the rep
controls entirely.

## Repository structure (ProspectLord-specific)
...
### POST /api/admin/case-studies/import-deck ✅ built
Admin-only. Accepts PDF upload. Converts pages to PNGs via `pdf-to-img`. Calls Claude vision per slide to extract metadata. Uploads qualifying slide PNGs to Supabase Storage bucket `case-study-slides`. Inserts records into `case_studies`. ⚠️ Uses dynamic import for `pdf-to-img` — see Next.js 16 notes in root CLAUDE.md.

### POST /api/case-studies/match ✅ built
...
### POST /api/case-studies/export-pdf ✅ built
...
- Use JSX in route handler `.ts` files — put components in `lib/pdf/*.tsx`, import and call via `React.createElement()` in the route
- Touch CELord code or shared `core/` / `signals/` code during ProspectLord work
